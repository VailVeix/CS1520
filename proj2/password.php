...
<?php
	...
	if(!isset($_POST['forgotuser'])){
		...
		$db = new mysqli('localhost', 'StiegelA', 'han-fred', 'StiegelA');
  		...
  		$result = $db->query("select Name from Makers");
  		...
  		if($found){
	  		...
	  		$query = "select Email from Makers where Name='" . $name . "'";
	  		$result = $db->query($query);
	  		$r = $result->fetch_array();
	  		$receive = $r['Email'];

	  		$query = "select Password from Makers where Name='" . $name . "'";
	  		$result = $db->query($query);
	  		$r = $result->fetch_array();
	  		$pass = $r['Password'];
	  		...
	  		header("location: /~acs119/php/assign2/password.php");
	  	}


  		echo "<a href='http://cs1520.cs.pitt.edu/~acs119/php/assign2/login.php'>Back to Login?</a>";
